/**
 * One-off: rename PDFs in public/courses/construction that contain " – " (en-dash) to use a single space.
 * Run from project root: npx tsx scripts/fix-construction-pdf-names.ts
 */
...
function main() {
  ...
  for (const file of files) {
    const newName = file
      .replace(/\s*[\u2013\u2014\-]\s*/g, " ") // en-dash, em-dash, or hyphen with spaces
      .replace(/\s+/g, " ")
      .trim();
